...
5. Optional: `WELCOME_EMAIL_CC` — comma-separated addresses **CC’d on the new-member welcome email only** (sent after full signup onboarding). If unset, defaults to `terry_bg@msn.com` and `Richard@richardleeweatherman.com`. The welcome send skips `EMAIL_STAFF_BCC` so those copies are not duplicated.
